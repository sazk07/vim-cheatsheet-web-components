...
const createCustomLists = (elemData, vcUl) => {
    ...
    for (const { keyword, additional_keyword2, additional_keyword3, description, } of commands) {
        const keywords = [
            keyword,
            additional_keyword2,
            additional_keyword3,
...
const main = async () => {
    ...
    const vimComponentNodes = Array.from(document.querySelectorAll("vim-component"));
    // get relevant objects from data
    const dataChild = vimComponentNodes
        .map((node) => retrieveObject(data, node));
    // Headings, lists and tips
    for (const elemData of dataChild) {
        const vimComponent = document.querySelector(`#${elemData.htmlId}`) ??
    ...
    anchor.setAttribute("href", "https://github.com/sazk07/vim-cheatsheet-web-components");
